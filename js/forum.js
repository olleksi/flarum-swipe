(function() {
    ...
    // ===== SWIPE LEFT/RIGHT ДЛЯ НАВІГАЦІЇ =====
    ...
    // ===== SWIPE LEFT/RIGHT ФУНКЦІОНАЛ =====

    function handleGlobalTouchStart(evt) {
        if (evt.target.closest('.Dropdown-toggle') || evt.target.closest('.navigation') || 
            evt.target.closest('.modal') || evt.target.closest('.Dropdown-menu') || 
            evt.target.closest('button') || evt.target.closest('textarea') || 
    ...
    function handleGlobalTouchMove(evt) {
        if (!xStart || !isSwiping) {
            return;
        }
    ...
    function initSwipe() {
        ...
        // Глобальні свайп обробники
        document.addEventListener('touchstart', handleGlobalTouchStart, false);
        document.addEventListener('touchmove', handleGlobalTouchMove, false);
        document.addEventListener('touchend', handleGlobalTouchEnd, false);
